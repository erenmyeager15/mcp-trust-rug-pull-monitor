...
Trusted and candidate snapshots are stored as versioned records in the default Key-Value Store only when the selected mode permits writes. Preserve the intended store across scheduled runs if you want later comparisons to reuse those records. Candidates carry their exact trusted-parent hash (or explicit no-parent state), and successful promotion deletes the consumed candidate.

Every trusted/candidate set or delete requires verified ownership of an atomic Apify Request Queue lock dedicated to that baseline identity. The queue contains one synthetic unique-key request with owner/expiry metadata; the Actor uses only Request Queue storage/locking APIs and never fetches its synthetic `.invalid` URL. Native lock expiry is longer than the maximum per-server mutation window, ownership is heartbeated while an overdue KVS mutation settles, and platform KVS mutation calls use a shorter non-retrying timeout. Expiry permits safe stale-owner takeover after a terminated run. If the queue API or ownership verification is unavailable, mutating modes fail closed; ordinary `compare_only` reads remain bounded, side-effect-free KVS reads. KVS records alone cannot provide cross-run atomicity, so deployments that cannot access Apify Request Queue APIs cannot mutate baseline state.
...
[`input-smoke.json`](input-smoke.json) is a no-secret cloud smoke configuration for the future public raw GitHub fixture URL `https://raw.githubusercontent.com/erenmyeager15/mcp-trust-rug-pull-monitor/master/src/__fixtures__/mcp-manifest.json`. It uses explicit `static_json`, `compare_only`, `dryRun`, no TLS/OSV/webhook, zero retries, one worker, a three-second request bound, and the minimum response limit. The fixture contains meaningful MCP server/tool/package metadata and no credentials or secret values. Once that repository path is public, a successful inspection is expected to return non-green `baseline_missing` with `baselineUpdated: false` and `candidateBaselineStored: false`; it performs no KVS or Request Queue writes. Endpoint availability before publication is external to this project.
